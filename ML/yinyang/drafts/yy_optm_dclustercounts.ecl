...
EXPORT yy_optm_dclustercounts  := MODULE
...
//%%%%%%%%%% code added for v4_update
    // dClusterCounts := TABLE(dUpperBound, {y; UNSIGNED c := COUNT(GROUP);},y );
		// dClusterCounts_pre := JOIN(dCentroid0, dClusterCounts, LEFT.id = RIGHT.y, TRANSFORM(RECORDOF(RIGHT), SELF.y := LEFT.id, SELF.c := IF(LEFT.id = RIGHT.y, RIGHT.c,0)), LEFT OUTER);
...
//%%%%%%%%%%% code added to v4_update
          ...
          action28 :=OUTPUT(dRolledVout, NAMED('YinyangKMeansv4_dRolledVout'));
					...
					dClusterCountsV :=JOIN(dClusterCountItr,dClusterCountsVin, LEFT.x = RIGHT.y, TRANSFORM(RECORDOF(LEFT), SELF.y := IF(LEFT.x = RIGHT.y,LEFT.y +RIGHT.c, LEFT.y);SELF.value := IF(LEFT.x = RIGHT.y,LEFT.value +RIGHT.c, LEFT.value); SELF := LEFT;), LEFT OUTER );
          action29 :=OUTPUT(dClusterCountsV, NAMED('YinyangKMeansv4_dClusterCountsV'));					
					dClusterCountsUpdate:=JOIN(dClusterCountsV,dClusterCountsVout, LEFT.x = RIGHT.y, TRANSFORM(RECORDOF(LEFT), SELF.y := IF(LEFT.x = RIGHT.y,LEFT.y -RIGHT.c, LEFT.y);SELF.value := IF(LEFT.x = RIGHT.y,LEFT.value - RIGHT.c, LEFT.value);  SELF := LEFT;), LEFT OUTER);
					action30 :=OUTPUT(dClusterCountsUpdate, NAMED('YinyangKMeansv4_dClusterCountsUpdate'));
					//Join to cluster counts to calculate the new average on each axis
					dClustered :=JOIN(dCentroidIn, dClusterCountItr, LEFT.id = RIGHT.x, TRANSFORM(RECORDOF(LEFT), SELF.value := IF(LEFT.id = RIGHT.x AND RIGHT.y != 0,LEFT.value * RIGHT.y, LEFT.value); SELF := LEFT), LEFT OUTER);
          action31 :=OUTPUT(dClustered, NAMED('YinyangKMeansv4_dClustered'));					
					//Add Vin 
					dRolled := JOIN(dClustered, dRolledVin, LEFT.id = RIGHT.id AND LEFT.number = RIGHT.number, TRANSFORM(RECORDOF(LEFT), SELF.value := IF(LEFT.id = RIGHT.id, LEFT.value + RIGHT.value, LEFT.value); SELF := LEFT), LEFT OUTER);
          action32 :=OUTPUT(dRolled, NAMED('YinyangKMeansv4_dRolled'));					
					//Minus Vout
					dJoined:= JOIN(dRolled, dRolledVout, LEFT.id = RIGHT.id AND LEFT.number = RIGHT.number, TRANSFORM(RECORDOF(LEFT), SELF.value := IF(LEFT.id = RIGHT.id,LEFT.value - RIGHT.value, LEFT.value); SELF := LEFT), LEFT OUTER);
					...
					dCentroid2_pre:=JOIN(dJoined,dClusterCountsUpdate,LEFT.id=RIGHT.x AND RIGHT.y != 0,TRANSFORM(RECORDOF(LEFT),SELF.value:= IF(LEFT.id = RIGHT.x AND RIGHT.y != 0, (LEFT.value/RIGHT.y), LEFT.value),SELF:=LEFT));
          	
										      // Find any centroids with no document allegiance and pass those through also
		  dPass:=JOIN(dCentroidIn,TABLE(dCentroid2_pre,{id},id),LEFT.id=RIGHT.id,TRANSFORM(LEFT),LEFT ONLY,LOOKUP);
					dCentroid2 := dJoined+dPass;
					action34 :=OUTPUT(dCentroid2, NAMED('YinyangKMeansv4_dCentroid2'));
*/

      dClusterCounts:=TABLE(dUbUpdate,{y;UNSIGNED c:=COUNT(GROUP);},y,FEW);
			action22 := OUTPUT(SORT(dClusterCounts,y), NAMED('dClusterCountsupdate'));
      // Join closest to the document set and replace the id with the centriod id
      ...
      // action23 := PARALLEL(action2, action3); 
					...
					// dClusterCountout := JOIN(dClusterCountsIn, dClusterCountsUpdate, LEFT.x = RIGHT.y ,TRANSFORM(lInput,SELF.id := 4;SELF.values:=IF(LEFT.x = RIGHT.x,LEFT.values+[RIGHT.value], LEFT.values);SELF.y := RIGHT.y;SELF.converge := bConverged; SELF.iter := c;SELF:=LEFT;));
			...
			// action := PARALLEL(action0, action22);	
			// RETURN dOutput;
			RETURN WHEN(dOutput, action22);
		END;
