//multigroup of centroids, no local filter and update step from yinyang: same code from v2combinev3.ecl
...
//combine YinyangKmeansv3 and yinyangkmeansv2 based on yinyangkmeansv2:test version workingon now 
...
EXPORT yinyangkmeansv3_update := MODULE
...
//%%%%%%%%%%% until next %%%%% , the code is the same
...
//%%%%%different part: get from yyv3
			deltaG1 := MAX(dDeltaC, value);
			action4 :=OUTPUT(deltaG1,NAMED('YinyangKMeans_deltaG1'));

//%%%%%%different part: get form yyv2:have to test if the dDeltaG is right
...
//%%%%%%different part : get dLbsGroupFilter
