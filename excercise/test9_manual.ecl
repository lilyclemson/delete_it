...
dCentroidMatrix := irisset.input[1..16];
...
Distances(DATASET(Types.NumericField) d01,DATASET(Types.NumericField) d02,DF.Default Control = DF.Euclidean) := FUNCTION
    ...
    // to their original numbers before returning the results.
		RETURN Result;
	END;
	 	 DistancesNew(DATASET(Types.NumericField) d01,DATASET(Types.NumericField) d02 ,DF.Default Control = DF.WEuclidean, DATASET(ClusterPair) dMap = DATASET([],ClusterPair)) := FUNCTION
   		ifMap := IF(COUNT(dMap)>0, TRUE, FALSE);		
   		// If we are in dense model then fatten up the records; otherwise zeroes not needed
   		df1 := IF( Control.Pmodel & c_model.dense > 0, Utils.Fat(d01), d01(value<>0) );
   		df2 := IF( Control.Pmodel & c_model.dense > 0, Utils.Fat(d02), d02(value<>0) );
   		// Construct the summary records used by SJoins and Background processing models
   		...
   		ex2 := Control.EV2(d01);
   			//Mapped dataset
   		df1_mapped := JOIN(dMap, df1, LEFT.id = RIGHT.id, TRANSFORM(ClusterPair, SELF.clusterid := LEFT.clusterid; SELF.value01:=RIGHT.value; SELF.value02:=0;SELF.value03:=0;SELF := RIGHT; ));//dRelation
   
   		ClusterPair Take2_mapped(ClusterPair le,df2 ri) := TRANSFORM
   				SELF.clusterid := ri.id;
   				SELF.id := le.id;
   				SELF.number := le.number;
   				SELF.value01 := Control.IV1(le.value01,ri.value);
   				SELF.value02 := Control.IV2(le.value01,ri.value);
   		END;
   			// This is the principle N^2 join (although some join filters can improve on that)
   		ClusterPair Take2(df1 le,df2 ri) := TRANSFORM
   				SELF.clusterid := ri.id;
   		...
   		J01 := JOIN(df1,df2,LEFT.number=RIGHT.number AND LEFT.id<>RIGHT.id AND Control.JoinFilter(LEFT.value,RIGHT.value,ex1),Take2(LEFT,RIGHT),HASH); // numbers will be evenly distribute by definition
   		J02 := JOIN(df1_mapped,df2,LEFT.number=RIGHT.number AND LEFT.id<>RIGHT.id AND Control.JoinFilter(LEFT.value01,RIGHT.value,ex1) AND LEFT.clusterid = RIGHT.id ,Take2_mapped(LEFT,RIGHT),HASH); // numbers will be evenly distribute by definition
   		J := IF(COUNT(dMap)>0,J02,J01);
   		// Take all of the values computed for each matching ID and combine them
   		...
   		J2 := JOIN(J1,si2,LEFT.clusterid=RIGHT.id,TRANSFORM(ClusterPair,SELF.value01 := Control.Join21(LEFT,RIGHT), SELF := LEFT),LOOKUP);
   		// Select either the 'normal' or 'post joined' version of the scores
   		Pro := IF ( Control.PModel & c_model.SJoins > 0 AND ifMap = FALSE, J2, rld );
   		ProAsDist := PROJECT(Pro,TRANSFORM(Mat.Types.Element,SELF.x := LEFT.id,SELF.y := LEFT.clusterid,SELF.Value := LEFT.Value01, SELF := LEFT));
   		// Now blend the scores that were computed with the background model
   		Mat.Types.Element blend(bck le,pro ri) := TRANSFORM
   		  SELF.value := Control.BackFront(le,ri);
   		...
   		BF := JOIN(bck,pro,LEFT.y=RIGHT.ClusterID AND LEFT.x=RIGHT.id,blend(LEFT,RIGHT),LEFT OUTER);
   		// Either select the background blended version - or slim the scores down to a cluster distance
       Result:=IF(Control.PModel & c_model.Background>0 AND ifMap = FALSE ,BF,ProAsDist);
  ...
  //---------------------------------------------------------------------------
  Closest(DATASET(Mat.Types.Element) dDistances):=DEDUP(SORT(DISTRIBUTE(dDistances,x),x,value,LOCAL),x,LOCAL);

	//*********************helper functions for calculate C'*****************************************
	// Function to pull iteration N from a table of type lIterations
	 Types.NumericField dResult(UNSIGNED n,DATASET(lIterations) d):=PROJECT(d,TRANSFORM(Types.NumericField,SELF.value:=LEFT.values[n+1];SELF:=LEFT;));
	...
	END;
	 ...
	 dDelta(UNSIGNED n01,UNSIGNED n02,DATASET(lIterations) d):=JOIN(dResult(n01,d),dResult(n02,d),LEFT.id=RIGHT.id AND LEFT.number=RIGHT.number,tGetDelta(LEFT,RIGHT));
			
	// Determine the distance delta between two iterations, using the distance
	// method specified by the user for this module
	// fDist :=DF.Euclidean;
	 dDistanceDelta(UNSIGNED n01,UNSIGNED n02,DATASET(lIterations) d, DF.Default fDist=DF.Euclidean ):=FUNCTION
...
// lInput yyfIterate(DATASET(lInput) d,UNSIGNED c):=FUNCTION
	//Get the distance from a data point to its second closest centroid
	...
	END;
			...
			deltac := dDistanceDelta(c,c-1,dAddedx, fDist);
	...
	dMap := PROJECT(unChangedUbSet, TRANSFORM(ClusterPair, SELF.id := LEFT.x; SELF.clusterid := LEFT.y;SELF.number := 0; SELF.value01 := LEFT.value; SELF.value02 := 0; SELF.value03 := 0;));
				OUTPUT(dMap, NAMED('dMap'));
				

			unChangedUbRolled := DistancesNew(d01,dCentroid1,fDist,dMap);
			OUTPUT(unChangedUbRolled, NAMED('unChangedUbRolled'));
...
*/
			lbs1_temp1 := SecondClosest(localFilter1,changelbstemp1);
			OUTPUT( lbs1_temp1, Named('lbs1_temp1'));
			//new lbs
			lbs1 := JOIN(lbs1_temp, lbs1_temp1,LEFT.x = RIGHT.x, TRANSFORM(Mat.Types.Element, SELF.value := IF( RIGHT.value = 0,LEFT.value, RIGHT.value); SELF := LEFT;), LEFT OUTER );
			...
			CV11:= JOIN(dCentroid1, V1, LEFT.id = RIGHT.x AND RIGHT.y = 0, TRANSFORM(LEFT));
			...
			//get the new C
			newC1:=JOIN(CV1VinVout,V1,LEFT.id=RIGHT.x AND RIGHT.y <> 0 ,TRANSFORM(RECORDOF(LEFT),SELF.value:=LEFT.value/RIGHT.y;SELF:=LEFT;));
			OUTPUT(newC1, NAMED('newC1'));
			
			newC:= newC1 + CV11;
			OUTPUT(newC, NAMED('newC'));
...
// lInput yyfIterate(DATASET(lInput) d,UNSIGNED c):=FUNCTION
			...
			changelbsTemp11111 := SORT(JOIN(localFilter11,changelbsTemp111, LEFT.x = RIGHT.x AND LEFT.y = RIGHT.y, RIGHT ONLY), x, value);
