...
EXPORT yinyangkmeansv4_update := MODULE
...
END;
  ...
  EXPORT YinyangKMeans(DATASET(Types.NumericField) d01,DATASET(Types.NumericField) d02,UNSIGNED n=1,REAL nConverge=0.0,DF.Default fDist=DF.Euclidean):=MODULE
    ...
    dClusterCounts := TABLE(dUpperBound, {y; UNSIGNED c := COUNT(GROUP);},y );
      ...
      action9 :=OUTPUT(dMap, NAMED('YinyangKMeansv4_dMap'));			
			dMappedDistances := MappedDistances(d01,dCentroidIn,fDist,dMap);
			action10 :=OUTPUT(dMappedDistances, NAMED('YinyangKMeansv4_dMappedDistances'));	
      ...
      action12 :=OUTPUT(groupFilter, NAMED('YinyangKMeansv4_groupFilter'));	
			...
			lemma2_r :=JOIN(gf_pass, dGroupDeltaC, LEFT.y = RIGHT.y, TRANSFORM(Mat.Types.Element, SELF.y := RIGHT.x ,SELF.value := RIGHT.y, SELF := LEFT));
			action13 :=OUTPUT(lemma2_r, NAMED('YinyangKMeansv4_llemma2_r'));    

      dMap_lemma2 := PROJECT(lemma2_r, TRANSFORM(ClusterPair, SELF.id := LEFT.x; SELF.clusterid := LEFT.y;SELF.number := 0; SELF.value01 := 0; SELF.value02 := 0; SELF.value03 := 0;));		
			action14 :=OUTPUT(dMap_lemma2, NAMED('YinyangKMeansv4_dMap_lemma2'));			
			dMappedDistances_lemma2 := MappedDistances(d01,dCentroidIn,fDist,dMap_lemma2);
			action15 :=OUTPUT(dMappedDistances_lemma2, NAMED('YinyangKMeansv4_dMappedDistances_lemma2'));	
			dClosestLocalFilter := DEDUP(SORT(dMappedDistances_lemma2,x,value),x);
			action16 :=OUTPUT(dClosestLocalFilter, NAMED('YinyangKMeansv4_dClosestLocalFilter'));
      ...
      action20 :=OUTPUT(LocalFilter, NAMED('YinyangKMeansv4_LocalFilter'));			
			dUbLocalFilter := JOIN(dMappedDistances,LocalFilter, LEFT.x = RIGHT.x, TRANSFORM(LEFT), LEFT ONLY);
			action21 :=OUTPUT(dUbLocalFilter, NAMED('YinyangKMeansv4_dUbLocalFilter'));	
			dUbUpdate:= LocalFilter + dUbLocalFilter;
			action22 :=OUTPUT(SORT(dUbUpdate,x), NAMED('YinyangKMeansv4_dUbUpdate'));
			
      lb_sub := JOIN(dMappedDistances_lemma2,LocalFilter, LEFT.x = RIGHT.x AND LEFT.y != RIGHT.y, TRANSFORM(RIGHT) );
		  action17:= OUTPUT(lb_sub, NAMED('lb_sub'));			

		  dGroupLbsLocalFilter := JOIN(lb_sub, Gt, LEFT.y = RIGHT.x, TRANSFORM(Mat.Types.Element, SELF.x := LEFT.x, SELF.y := RIGHT.y, SELF.value := LEFT.value));
			action18 :=OUTPUT(dGroupLbsLocalFilter, NAMED('YinyangKMeansv4_dGroupLbsLocalFilter'));		  
		  dGroupLbsUpdate := DEDUP(SORT(dGroupLbsLocalFilter, x, y, value),LEFT.x = RIGHT.x AND LEFT.y = RIGHT.y);		
      action19 :=OUTPUT(dGroupLbsUpdate, NAMED('YinyangKMeansv4_dGroupLbsUpdate')); 
			...
			dLbsUpdate_pre := JOIN(dLbsGroupFilter, dGroupLbsUpdate, LEFT.x = RIGHT.x AND LEFT.y = RIGHT.y, TRANSFORM(RECORDOF(LEFT), SELF := LEFT),LEFT ONLY);
      action23 :=OUTPUT(SORT(dLbsUpdate_pre,x), NAMED('dLbsUpdate_pre'));

				dLbsUpdate := dLbsUpdate_pre + dGroupLbsUpdate;

				action24 :=OUTPUT(SORT(dLbsUpdate,x), NAMED('dLbsupdate'));
					...
					action25 :=OUTPUT(dClusterCountsVin, NAMED('YinyangKMeansv4_dClusterCountsVin'));
					// Join closest to the document set and replace the id with the centriod id
					dClusteredVin := JOIN(d01, LocalFilter, LEFT.id = RIGHT.x,TRANSFORM(Types.NumericField,SELF.id:=RIGHT.y;SELF:=LEFT;));
					action26 :=OUTPUT(dClusteredVin, NAMED('YinyangKMeansv4_dClusteredVin'));
					// Now roll up on centroid ID, summing up the values for each axis
					...
					action27 :=OUTPUT(dRolledVin, NAMED('YinyangKMeansv4_dRolledVin'));
					...
					action28 :=OUTPUT(VoutSet, NAMED('YinyangKMeansv4_VoutSet'));
					dClusterCountsVout:=TABLE(VoutSet, {y; INTEGER c := COUNT(GROUP)},y);
					action29 :=OUTPUT(dClusterCountsVout, NAMED('YinyangKMeansv4_dClusterCountsVout'));
					
					//Join closest to the document set and replace the id with the centriod id
					dClusteredVout := JOIN(d01,VoutSet, LEFT.id = RIGHT.x,TRANSFORM(Types.NumericField,SELF.id:=RIGHT.y;SELF:=LEFT;));
					action30 :=OUTPUT(dClusteredVout, NAMED('YinyangKMeansv4_dClusteredVout'));
          ...
          action31 :=OUTPUT(dRolledVout, NAMED('YinyangKMeansv4_dRolledVout'));
          ...
          action32 :=OUTPUT(dClusterCountsV, NAMED('YinyangKMeansv4_dClusterCountsV'));					
					...
					action33 :=OUTPUT(dClusterCountsUpdate, NAMED('YinyangKMeansv4_dClusterCountsUpdate'));
					//Join to cluster counts to calculate the new average on each axis
          ...
          action34 :=OUTPUT(dClustered, NAMED('YinyangKMeansv4_dClustered'));					
					//Add Vin 
          ...
          action35 :=OUTPUT(dRolled, NAMED('YinyangKMeansv4_dRolled'));					
					//Minus Vout
					...
					action36 :=OUTPUT(dJoined, NAMED('YinyangKMeansv4_dJoined'));	
					//New C
          ...
          action37 :=OUTPUT(dCentroid2, NAMED('YinyangKMeansv4_dCentroid2'));	
    			...
    			// action := PARALLEL(output_dGroupDeltaC,action66,action00,action0,action1,action2, action3,action4,action5,action6,action7,action8,action9,action10,action11,action12,action13,action14,action15,action16,action17,action18,action19,action20,action21,action22,action23,4,action25,action26,action27,action28,action29,action30,action31,action32,action33,action34,action35, action36, action37);

			
			action := PARALLEL(action0, action33);	
			// RETURN dOutput;
