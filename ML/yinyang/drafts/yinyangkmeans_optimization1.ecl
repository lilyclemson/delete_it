...
EXPORT yinyangkmeans_optimization1:= MODULE
...
EXPORT YinyangKMeans(DATASET(Types.NumericField) d01,DATASET(Types.NumericField) d02,DATASET(Types.NumericField) d03,UNSIGNED n=1,REAL nConverge=0.0,DF.Default fDist=DF.Euclidean):=MODULE
...
//Closest() distribute all the distances based on X on each node. -- the first manual distribute 		
//on each node, the data points are sorted by x, value and then dedup based on x
		dUpperBound := Closest(dDistances);
		output_dUpperBound := OUTPUT(dUpperBound, NAMED('dUpperBound'));
		//dUpperBound is local based on x, dDistances is local based on x, dUpperBound is smaller than dDistances,  LOCAL 
			dDistancesSub := JOIN(dDistances, dUpperBound, LEFT.x = RIGHT.x AND LEFT.y = RIGHT.y,LEFT ONLY, LOCAL);
    ...
    KmeansDt := KMeans(dCentroid0,d03,nt,tConverge);
    ...
    // will lookup join change the distribute of the data? 
    dLowerBound := DEDUP( SORT(DISTRIBUTE(dGroupDistancesSub,x),x, y, value, LOCAL),LEFT.x = RIGHT.x AND LEFT.y = RIGHT.y, LOCAL);
    output_dLowerBound := OUTPUT(dLowerBound, NAMED('dLowerBound'));
		...
		action_counits_ini := OUTPUT(dClusterCounts_ini, NAMED('dClusterCounts_ini'));
    ...
    // dClusterCounts := TABLE(dUpperBound, {y; UNSIGNED c := COUNT(GROUP);},y );
       ...
       action0:= OUTPUT(dCentroidIn, NAMED('dCentroidIn'));
	   ...
	   // dClusterCountsIn := TABLE(d(id = 4), {x;y;values;});
			...
			dUbItr := DISTRIBUTE(PROJECT(d(id = 2), TRANSFORM(Mat.Types.Element, SELF.value := LEFT.values[c]; SELF := LEFT;)),x);
			action1 := OUTPUT(SORT(dUbItr,x), NAMED('YinyangKMeans_dUbItr'));
			dLbsItr := DISTRIBUTE(PROJECT(d(id = 3), TRANSFORM(Mat.Types.Element, SELF.value := LEFT.values[c]; SELF := LEFT;)),x);
			...
			dGroupDeltaC :=JOIN(dDeltaC, Gt, LEFT.id = RIGHT.x, TRANSFORM(Mat.Types.Element,SELF.value := LEFT.value; SELF := RIGHT;), LOOKUP);
			output_dGroupDeltaC := OUTPUT(dGroupDeltaC, NAMED('dGroupDeltaC'));
		  dDeltaG_pre := TABLE(dGroupDeltaC,{y,value},y, value);
			output_dDeltaG_Pre := OUTPUT(dDeltaG_pre, NAMED('dDeltaG_pre'));
			dDeltaG := DEDUP(dDeltaG_pre,y,RIGHT);
			action5 :=OUTPUT(dDeltaG,NAMED('YinyangKMeans_dDeltaG')); 

			deltaG1 := DEDUP(SORT(dLbsItr,x,value, LOCAL), x, LOCAL);
			action4 :=OUTPUT(deltaG1,NAMED('YinyangKMeans_deltaG1'));

			dUbGroupFilter := JOIN(dUbItr, dDeltaC, LEFT.y = RIGHT.id, TRANSFORM(Mat.Types.Element, SElF.value := LEFT.value + RIGHT.value; SELF := LEFT;), LOOKUP);
      action6 :=OUTPUT(dUbGroupFilter, NAMED('YinyangKMeans_dUbGroupFilter'));

      //
			dUbGroupFilter_glb := JOIN(deltaG1,dUbGroupFilter,  LEFT.x = right.x AND LEFT.value < RIGHT.value, TRANSFORM(LEFT), LOCAL);
			action66 := OUTPUT(dUbGroupFilter_glb, NAMED('dUbGroupFilter_glb'));
      ...
      action7 :=OUTPUT(dLbsGroupFilter, NAMED('YinyangKMeans_dLbsGroupFilter'));			
			...
			gf_pass_pre := JOIN(dUbGroupFilter_glb, dLbsGroupFilter,LEFT.x = RIGHT.x AND LEFT.value > RIGHT.value, TRANSFORM(RIGHT), LOCAL);
      action8 := OUTPUT(gf_pass_pre, NAMED('YinyangKMeansv4_gf_pass_pre'));
			
			dMap_gf := PROJECT(dUbItr, TRANSFORM(ClusterPair, SELF.id := LEFT.x; SELF.clusterid := LEFT.y;SELF.number := 0; SELF.value01 := LEFT.value; SELF.value02 := 0; SELF.value03 := 0;));		
      action9 :=OUTPUT(dMap_gf, NAMED('YinyangKMeansv4_dMap_gf'));			
			dMappedDistances_gf := MappedDistances(d01,dCentroidIn,fDist,dMap_gf);
			action10 :=OUTPUT(dMappedDistances_gf, NAMED('YinyangKMeansv4_dMappedDistances_gf'));	

			
      gf_pass := JOIN(gf_pass_pre, DISTRIBUTE(dMappedDistances_gf,x), LEFT.x = RIGHT.x AND LEFT.value < RIGHT.value, TRANSFORM(LEFT), LOCAL);
			action11 := OUTPUT(gf_pass, NAMED('YinyangKMeansv4_gf_pass'));
			groupFilter := DEDUP(gf_pass,x, LOCAL);
      action12 :=OUTPUT(groupFilter, NAMED('YinyangKMeansv4_groupFilter'));	


			dLocalFilter := JOIN(DISTRIBUTE(d01,id),groupFilter, LEFT.id = RIGHT.x,TRANSFORM(LEFT),LOCAL);
	    ...
	    LocalFilter := JOIN(dClosestLocalFilter,dUbItr, LEFT.x = RIGHT.x AND LEFT.y !=RIGHT.y, TRANSFORM(LEFT), LOCAL);
      action16 :=OUTPUT(LocalFilter, NAMED('YinyangKMeansv4_LocalFilter'));			
			dUbLocalFilter := JOIN(dUbItr, LocalFilter, LEFT.x = RIGHT.x, TRANSFORM(LEFT), LEFT ONLY, LOCAL);
      ...
      action18 :=OUTPUT(dMap, NAMED('YinyangKMeansv4_dMap'));			
			...
			dLbsLocalFilter := JOIN(Distribute(dDistancesLocalFilter,x), LocalFilter, LEFT.x = RIGHT.x AND LEFT.y != RIGHT.y, TRANSFORM(LEFT),LOCAL);
			dGroupLbsLocalFilter := SORT(JOIN(dLbsLocalFilter, Gt, LEFT.y = RIGHT.x, TRANSFORM(Mat.Types.Element,SELF.y := RIGHT.y, SELF := LEFT), MANY LOOKUP),x, y, value, LOCAL);		
		  dGroupLbsUpdate := DEDUP(dGroupLbsLocalFilter,LEFT.x = RIGHT.x AND LEFT.y = RIGHT.y, LOCAL);      
			...
			dLbsUpdate := JOIN(dLbsGroupFilter, dGroupLbsUpdate,LEFT.x = RIGHT.x AND LEFT.y = RIGHT.y, TRANSFORM(Mat.Types.Element, SELF.value := RIGHT.value, SELF := LEFT), LEFT OUTER, LOCAL );
...
/*					//Calculate Vin: the number of data points who move into a new cluster					
					dClusterCountsVin:=TABLE(LocalFilter, {y; INTEGER c := COUNT(GROUP)},y,FEW, LOCAL);
					action22 :=OUTPUT(dClusterCountsVin, NAMED('YinyangKMeansv4_dClusterCountsVin'));
					// Join closest to the document set and replace the id with the centriod id
					dClusteredVin := JOIN(d01, LocalFilter, LEFT.id = RIGHT.x,TRANSFORM(Types.NumericField,SELF.id:=RIGHT.y;SELF:=LEFT;), LOOKUP,LOCAL);
					action23 :=OUTPUT(dClusteredVin, NAMED('YinyangKMeansv4_dClusteredVin'));
					// Now roll up on centroid ID, summing up the values for each axis
					dRolledVin:=ROLLUP(SORT(dClusteredVin,id, number, LOCAL),LEFT.number = RIGHT.number,TRANSFORM(Types.NumericField,SELF.value:=LEFT.value+RIGHT.value;SELF:=LEFT;), LOCAL);
					...
					VoutSet := JOIN(dUbGroupFilter, LocalFilter, LEFT.x = RIGHT.x, TRANSFORM(LEFT), LOCAL);
					action25 :=OUTPUT(VoutSet, NAMED('YinyangKMeansv4_VoutSet'));
					dClusterCountsVout:=TABLE(VoutSet, {y; INTEGER c := COUNT(GROUP)},y, FEW, LOCAL);
					action26 :=OUTPUT(dClusterCountsVout, NAMED('YinyangKMeansv4_dClusterCountsVout'));
					
					//Join closest to the document set and replace the id with the centriod id
					dClusteredVout := JOIN(d01,VoutSet, LEFT.id = RIGHT.x,TRANSFORM(Types.NumericField,SELF.id:=RIGHT.y;SELF:=LEFT;), LOCAL);
					action27 :=OUTPUT(dClusteredVout, NAMED('YinyangKMeansv4_dClusteredVout'));
					
					// Now roll up on centroid ID, summing up the values for each axis
					dRolledVout:=ROLLUP(SORT(dClusteredVout,id, number, LOCAL),LEFT.number = RIGHT.number,TRANSFORM(Types.NumericField,SELF.value:=LEFT.value+RIGHT.value;SELF:=LEFT;), LOCAL);
...
*/
      ...
      dClusterCountsupdate:=TABLE(dUbUpdate,{y;UNSIGNED c:=COUNT(GROUP);},y,FEW);
			action22 := OUTPUT(SORT(dClusterCountsupdate,y), NAMED('dClusterCountsupdate'));
      // Join closest to the document set and replace the id with the centriod id
      dClustered:=SORT(DISTRIBUTE(JOIN(d01,dUbUpdate,LEFT.id=RIGHT.x,TRANSFORM(Types.NumericField,SELF.id:=RIGHT.y;SELF:=LEFT;),HASH),id),RECORD,LOCAL);
      // Now roll up on centroid ID, summing up the values for each axis
      dRolled:=ROLLUP(dClustered,TRANSFORM(Types.NumericField,SELF.value:=LEFT.value+RIGHT.value;SELF:=LEFT;),id,number,LOCAL);
      // Join to cluster counts to calculate the new average on each axis
      dJoined:=JOIN(dRolled,dClusterCountsupdate,LEFT.id=RIGHT.y,TRANSFORM(Types.NumericField,SELF.value:=LEFT.value/RIGHT.c;SELF:=LEFT;),LOOKUP);
      // Find any centroids with no document allegiance and pass those through also
		  dPass:=JOIN(dCentroidIn,TABLE(dJoined,{id},id,LOCAL),LEFT.id=RIGHT.id,TRANSFORM(LEFT),LEFT ONLY,LOOKUP);
      // Now join to the existing centroid dataset to add the new values to
      // the end of the values set.
      dCentroid2 := dJoined+dPass;
      // action23 := PARALLEL(action2, action3); 
					...
					// dClusterCountout := JOIN(dClusterCountsIn, dClusterCountsUpdate, LEFT.x = RIGHT.y,TRANSFORM(lInput,SELF.id := 4;SELF.values:=LEFT.values+[RIGHT.value];SELF.y := RIGHT.y;SELF.converge := bConverged; SELF.iter := c;SELF:=LEFT;));
					SHARED dOutput := dCentroidsOut+ dUbOut + dLbsOut ;
			
    			// action := PARALLEL(output_dDistances0, output_dUpperBound, output_Gt, output_groups, output_dLowerBound, action00,action0,action1,action2, action3,output_dGroupDeltaC,action5,action6,action7,action8,action9,action10,action11,action12,action13,action14,action15,action16,action17,action18,action19,action20,action21,action22,action24,action25,action26,action27,action28,action29,action30,action31,action32,action33, action34);
			
    			// action := PARALLEL(output_dDistances0, output_dDeltaG_Pre, output_dUpperBound, output_Gt, output_groups, output_dLowerBound, action00,action0,action1,action2, action3,output_dGroupDeltaC,action5,action6,action7,action8,action9,action10,action11,action12,action19,action20,action21,action22);			
			// action := PARALLEL(action1, action20, action22,action0,action_counits_ini);	
			RETURN dOutput;
