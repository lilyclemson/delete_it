...
// ave-dist - average of the components
  ...
  //              centroid allegiance.  Default is simple Euclidean.
  //---------------------------------------------------------------------------
SHARED	SimpleDistances(DATASET(Types.NumericField) d01,DATASET(Types.NumericField) d02) := FUNCTION
...
SHARED Drift(DATASET(Types.NumericField) d01,DATASET(Types.NumericField) d02) := FUNCTION
	...
	dDelta1:= TABLE(dDistance_pwr,{id, val:=SQRT(SUM(GROUP,value));},id); 
	RETURN dDelta1; 
END;
  ...
  EXPORT YinyangKMeans(DATASET(Types.NumericField) d01,DATASET(Types.NumericField) d02,UNSIGNED n=1,REAL nConverge=0.0,DF.Default fDist=DF.Euclidean):=MODULE
	  ...
	  d02Prep:=PROJECT(d02,TRANSFORM(lIterations,SELF.id:=LEFT.id+iOffset;SELF.values:=[LEFT.value];SELF:=LEFT;));
		...
		dDistances := SimpleDistances(d01,dCentroid0); 
    ...
    //id = x.id, cluterid = c.id number = g.id value01 = lowerbound value02 = 0 value03 = 0
    // dGroupDistancesSub := SORT(JOIN(dDistancesSub, Gt, LEFT.y = RIGHT.x, TRANSFORM(Mat.Types.Element,SELF.y := RIGHT.y, SELF := LEFT)),x, y, value);		
		...
		dGroupDistancesSub := SORT(JOIN(dDistancesSub, Gt, LEFT.y = RIGHT.x, TRANSFORM(ClusterPair,SELF.id := LEFT.x,SELF.clusterid := LEFT.y, SELF.number := RIGHT.y, SELF.value01 := LEFT.value, SELF.value02 := 0, SELF.value03 := 0)),id, number, value01);		
		output_dGroupDistancesSub := OUTPUT(dGroupDistancesSub, NAMED('dGroupDistancesSub'));
		
		dLowerBound_pre := DEDUP(dGroupDistancesSub,LEFT.id = RIGHT.id AND LEFT.number = RIGHT.number);
		output_dLowerBound_pre := OUTPUT(dLowerBound_pre, NAMED('dLowerBound_pre')); 
		
		dLowerBound := PROJECT(dLowerBound_pre, TRANSFORM(Mat.Types.Element, SELF.x := LEFT.id, SELF.y := LEFT.number, SELF.value := LEFT.value01));
    output_dLowerBound := OUTPUT(dLowerBound, NAMED('dLowerBound'));

		dLowerBound_centroid := PROJECT(dLowerBound_pre, TRANSFORM(Mat.Types.Element, SELF.x:= LEFT.id, SELF.y := LEFT.number , SELF.value := LEFT.clusterid ));
    output_dLowerBound_centroid := OUTPUT(dLowerBound_centroid, NAMED('dLowerBound_centroid'));
    ...
    dLowerBound_centroidPre := PROJECT(dLowerBound_centroid, transformat(LEFT, 5));		
		dInput := dCentroidsPrep + dUbPrep + dLbsPrep + dClusterCountsPrep+ dLowerBound_centroidpre;
		
		
		lInput fIterate(DATASET(lInput) d,UNSIGNED c):=FUNCTION	
		  action00 := OUTPUT(c, NAMED('iteration'));
			...
			dLowerBound_centroidIn := TABLE(d(id = 5), {x;y;values;});
			action_dLowerBound_centroidIn := OUTPUT(dLowerBound_centroidIn, NAMED('dLowerBound_centroidIn'));
			...
			action0 := OUTPUT(dClusterCountItr, NAMED('YinyangKMeans_dClusterCountItr'));
			...
			dLowerBound_centroidItr := PROJECT(d(id =5),TRANSFORM(Mat.Types.Element, SELF.value := LEFT.y, SELF.y := LEFT.values[c]; SELF := LEFT;) );
      
			//id = x.id, clusterid = c.id number = g.id value01 = lowerbound
			dLowerBound_gf_pass := JOIN(dLowerBound_centroidItr,  dLbsItr, LEFT.x = Right.x and left.value = right.y, TRANSFORM(ClusterPair, SELF.id := LEFT.x, SELF.clusterid := LEFT.y, SELF.number := LEFT.value, SELF.value01:= right.value, SELF.value02 := 0, SELF.value03:= 0) );
			action_dLowBound_gf_pass := OUTPUT(dLowerBound_gf_pass, NAMED('dLowerBound_gf_pass'));
			action_dLowerBound_centroidItr := OUTPUT(dLowerBound_centroidItr, NAMED('dLowerBound_centroidItr'));
			
			dDeltaC := TABLE(Drift(dResult(c,dCentroidsIn), dResult(c-1,dCentroidsIn)), {id, value := val;});
			action3 :=OUTPUT(dDeltaC,NAMED('YinyangKMeans_dDeltaC'));

			deltaG1 := DEDUP(SORT(dLbsItr, x,value), x);
      ...
      dUbGroupFilter_glb := JOIN(dUbGroupFilter, deltaG1, LEFT.x = right.x AND LEFT.value > RIGHT.value, TRANSFORM(LEFT));
			output_dUbGroupFilter_glb := OUTPUT(dUbGroupFilter_glb, NAMED('dUbGroupFilter_glb'));
      ...
      action7 :=OUTPUT(dLbsGroupFilter, NAMED('YinyangKMeans_dLbsGroupFilter'));			
			...
			gf_pass_pre := JOIN(dUbGroupFilter_glb, dLbsGroupFilter,LEFT.x = RIGHT.x AND LEFT.value > RIGHT.value, TRANSFORM(RIGHT));
      action_gf_pass_pre := OUTPUT(gf_pass_pre, NAMED('YinyangKMeansv4_gf_pass_pre'));
			
			dMap := PROJECT(dUbItr, TRANSFORM(ClusterPair, SELF.id := LEFT.x; SELF.clusterid := LEFT.y;SELF.number := 0; SELF.value01 := LEFT.value; SELF.value02 := 0; SELF.value03 := 0;));		
      ...
      gf_pass := JOIN(gf_pass_pre, dMappedDistances, LEFT.x = RIGHT.x AND LEFT.value < RIGHT.value, TRANSFORM(LEFT));
			action_gf_pass := OUTPUT(gf_pass, NAMED('YinyangKMeansv4_gf_pass'));
			
			
			groupFilter := DEDUP(gf_pass,x);
      action8 :=OUTPUT(groupFilter, NAMED('YinyangKMeansv4_groupFilter'));	
      
			// JOIN(gf_pass, dLbsItr, LEFT.x = RIGHT.x AND LEFT.y = RIGHT.y, TRANSFORM());
		 ...
		 //id = x.id clusterid = c.id (lowerbound) number = g.id value01 =lowerbound, value02= lbgroupfilter
			gf_pass_centroid := JOIN(gf_pass, dLowerBound_gf_pass, left.x = right.id AND left.y = right.number, TRANSFORM(ClusterPair,SELF.value02 := LEFT.value ,SELF := RIGHT));
      output_gf_pass_centroid := OUTPUT(gf_pass_centroid, NAMED('gf_pass_centroid'));
			
			// lemma2_r :=JOIN(gf_pass, dGroupDeltaC, LEFT.y = RIGHT.y, TRANSFORM(Mat.Types.Element, SELF.y := RIGHT.x ,SELF.value := LEFT.value - RIGHT.value, SELF := LEFT));
     //x = x.id y=  value =     
		 lemma2_r :=JOIN(gf_pass_centroid, dGroupDeltaC, LEFT.number = RIGHT.y , TRANSFORM(Mat.Types.Element, SELF.x := LEFT.id, SELF.y := RIGHT.x ,SELF.value := LEFT.value02 - RIGHT.value));      
			action11 :=OUTPUT(lemma2_r, NAMED('YinyangKMeansv4_llemma2_r'));    
      lemma2_pre := SORT(JOIN(lemma2_r, lemma2_l_glb, LEFT.x = RIGHT.x AND LEFT.value <= RIGHT.value, TRANSFORM(LEFT)),x);
			action_lemma2_pre := OUTPUT(lemma2_pre, named('lemma2_pre'));
//method1 : 

			lemma2 := DEDUP(SORT(JOIN(lemma2_r, lemma2_l_glb, LEFT.x = RIGHT.x AND LEFT.value <= RIGHT.value, TRANSFORM(LEFT)),x),x);	      
			action12 :=OUTPUT(lemma2, NAMED('YinyangKMeansv4_lemma2'));			

	
      dMap_lemma2 := PROJECT(lemma2_pre, TRANSFORM(ClusterPair, SELF.id := LEFT.x; SELF.clusterid := LEFT.y;SELF.number := 0; SELF.value01 := LEFT.value; SELF.value02 := 0; SELF.value03 := 0;));		
			action13 :=OUTPUT(dMap_lemma2, NAMED('YinyangKMeansv4_dMap_lemma2'));			
			dMappedDistances_lemma2 := MappedDistances(d01,dCentroidIn,fDist,dMap_lemma2);
			action14 :=OUTPUT(dMappedDistances_lemma2, NAMED('YinyangKMeansv4_dMappedDistances_lemma2'));	
			
			// dClosestLocalFilter := Closest(dMappedDistances_lemma2);
			dClosestLocalFilter := DEDUP(SORT(dMappedDistances_lemma2,x,value),x);
			action15 :=OUTPUT(dClosestLocalFilter, NAMED('YinyangKMeansv4_dClosestLocalFilter'));

	    LocalFilter := JOIN(dClosestLocalFilter,dMappedDistances, LEFT.x = RIGHT.x AND LEFT.value < RIGHT.value, TRANSFORM(LEFT), LOOKUP, FEW);
      action16 :=OUTPUT(LocalFilter, NAMED('YinyangKMeansv4_LocalFilter'));			
			dUbLocalFilter := JOIN(dMappedDistances,LocalFilter, LEFT.x = RIGHT.x, TRANSFORM(LEFT), LEFT ONLY);
			action17 :=OUTPUT(dUbLocalFilter, NAMED('YinyangKMeansv4_dUbLocalFilter'));	
	
			dUbUpdate:= LocalFilter + dUbLocalFilter;
			action20 :=OUTPUT(SORT(dUbUpdate,x), NAMED('YinyangKMeansv4_dUbUpdate'));
//added	**
      group_change := JOIN(LocalFilter, Gt, LEFT.y = RIGHT.x, TRANSFORM(ClusterPair, SELF.id := LEFT.x, SELF.clusterid := LEFT.y, SELF.number := RIGHT.y,  SELF.value01 := 0, SELF.value02 := 0, SELF.value03 := 0) );
			dMap_lb_pre := JOIN(group_change, Gt, LEFT.number = RIGHT.y, TRANSFORM(ClusterPair,  SELF.clusterid := RIGHT.x, SELF := LEFT)); 	
			dMap_lb := JOIN(dMap_lb_pre, LocalFilter, LEFT.id = RIGHT.x AND LEFT.clusterid = RIGHT.y, TRANSFORM(LEFT), LEFT ONLY );
			dMappedDistance_lb := MappedDistances(d01, dCentroidIn, fDist, dMap_lb);
      dLbsLocalFilter := dMappedDistance_lb;
//

		dGroupLbsLocalFilter := SORT(JOIN(dLbsLocalFilter, Gt, LEFT.y = RIGHT.x, TRANSFORM(ClusterPair,SELF.id := LEFT.x,SELF.clusterid := LEFT.y, SELF.number := RIGHT.y, SELF.value01 := LEFT.value, SELF.value02 := 0, SELF.value03 := 0)),id, number, value01);		

		dGroupLbsUpdate_pre := DEDUP(dGroupLbsLocalFilter,LEFT.id = RIGHT.id AND LEFT.number = RIGHT.number);

		dGroupLbsUpdate := PROJECT(dGroupLbsUpdate_pre, TRANSFORM(Mat.Types.Element, SELF.x := LEFT.id, SELF.y := LEFT.number, SELF.value := LEFT.value01));
   
		dLowerBound_centroid_update_pre := PROJECT(dGroupLbsUpdate_pre, TRANSFORM(Mat.Types.Element, SELF.x:= LEFT.id, SELF.y := LEFT.number , SELF.value := LEFT.clusterid ));
    dLowerBound_centroid_update := JOIN(dLowerBound_centroidItr,dLowerBound_centroid_update_pre, LEFT.x = RIGHT.x AND LEFT.value = RIGHT.y, TRANSFORM(Mat.Types.Element, self.x:= left.x, self.y := IF(LEFT.x = RIGHT.x, RIGHT.y, LEFT.value), SELF.value := IF(LEFT.x = RIGHT.x, RIGHT.value,LEFT.y)), LEFT OUTER);
		output_dLowerBound_centroid_update_pre := OUTPUT(dLowerBound_centroid_update_pre, NAMED('dLowerBound_centroid_update_pre'));
	  output_dLowerBound_centroid_update := OUTPUT(dLowerBound_centroid_update, NAMED('dLowerBound_centroid_update'));
		 ...
		 // dGroupLbsUpdate := DEDUP(dGroupLbsLocalFilter,LEFT.x = RIGHT.x AND LEFT.y = RIGHT.y);      
			...
			action35 :=OUTPUT(dLbsLocalFilter, NAMED('YinyangKMeansv4_dLbsLocalFilter'));		  

      action36 :=OUTPUT(dGroupLbsLocalFilter, NAMED('YinyangKMeansv4_dGroupLbsLocalFilter'));		  
  
      action37 :=OUTPUT(dGroupLbsUpdate, NAMED('YinyangKMeansv4_dGroupLbsUpdate'));


			dLbsUpdate_pre := JOIN(dLbsGroupFilter, dGroupLbsUpdate,LEFT.x = RIGHT.x AND LEFT.y = RIGHT.y, TRANSFORM(LEFT), LEFT ONLY );
      action_dLbsUpdate_pre :=OUTPUT(SORT(dLbsUpdate_pre,x), NAMED('dLbsUpdate1'));

				dLbsUpdate := dLbsUpdate_pre + dGroupLbsUpdate;

				action21 :=OUTPUT(SORT(dLbsUpdate,x));
					...
					action22 :=OUTPUT(dClusterCountsVin, NAMED('YinyangKMeansv4_dClusterCountsVin'));
					// Join closest to the document set and replace the id with the centriod id
					dClusteredVin := JOIN(d01, LocalFilter, LEFT.id = RIGHT.x,TRANSFORM(Types.NumericField,SELF.id:=RIGHT.y;SELF:=LEFT;));
					action23 :=OUTPUT(dClusteredVin, NAMED('YinyangKMeansv4_dClusteredVin'));
					// Now roll up on centroid ID, summing up the values for each axis
					...
					action24 :=OUTPUT(dRolledVin, NAMED('YinyangKMeansv4_dRolledVin'));
					...
					action25 :=OUTPUT(VoutSet, NAMED('YinyangKMeansv4_VoutSet'));
					dClusterCountsVout:=TABLE(VoutSet, {y; INTEGER c := COUNT(GROUP)},y);
					action26 :=OUTPUT(dClusterCountsVout, NAMED('YinyangKMeansv4_dClusterCountsVout'));
					
					//Join closest to the document set and replace the id with the centriod id
					dClusteredVout := JOIN(d01,VoutSet, LEFT.id = RIGHT.x,TRANSFORM(Types.NumericField,SELF.id:=RIGHT.y;SELF:=LEFT;));
					action27 :=OUTPUT(dClusteredVout, NAMED('YinyangKMeansv4_dClusteredVout'));
          ...
          action28 :=OUTPUT(dRolledVout, NAMED('YinyangKMeansv4_dRolledVout'));
          ...
          action29 :=OUTPUT(dClusterCountsV, NAMED('YinyangKMeansv4_dClusterCountsV'));					
					...
					action30 :=OUTPUT(dClusterCountsUpdate, NAMED('YinyangKMeansv4_dClusterCountsUpdate'));
					//Join to cluster counts to calculate the new average on each axis
          ...
          action31 :=OUTPUT(dClustered, NAMED('YinyangKMeansv4_dClustered'));					
					//Add Vin 
          ...
          action32 :=OUTPUT(dRolled, NAMED('YinyangKMeansv4_dRolled'));					
					//Minus Vout
					...
					action33 :=OUTPUT(dJoined, NAMED('YinyangKMeansv4_dJoined'));	
					//New C
          ...
          action34 :=OUTPUT(dCentroid2, NAMED('YinyangKMeansv4_dCentroid2'));	
					...
					dLowerBound_centroid_out := JOIN(dLowerBound_centroidIn, dLowerBound_centroid_update, LEFT.x = RIGHT.x and LEFT.y = RIGHT.y ,TRANSFORM(lInput,SELF.id := 5;SELF.values:=LEFT.values+[RIGHT.value]; SELF.converge := bConverged; SELF.iter := c;SELF:=LEFT;) );
					
					SHARED dOutput := dCentroidsOut+ dUbOut + dLbsOut + dClusterCountout + dLowerBound_centroid_out;
			
    			action := PARALLEL(action_gf_pass_pre,output_dUbGroupFilter_glb,output_dLowerBound_centroid_update_pre,output_dLowerBound_centroid_update,output_dGroupDistancesSub,output_gf_pass_centroid,action_dLowerBound_centroidIn, output_dLowerBound_pre,output_dLowerBound_centroid,action_dLowerBound_centroidItr,output_dGroupDeltaC,output_dUpperBound, output_Gt, output_groups,action_dLowBound_gf_pass, output_dLowerBound, action00,action0,action1,action2, action3,action4,action5,action6,action7,action8,action_gf_pass,action10,action11,action12,action_lemma2_pre, action13,action14,action15,action16,action17,action18,action19,action20,action_dLbsUpdate_pre,action35, action36, action37, action21,action22,action23,action24,action25,action26,action27,action28,action29,action30,action31,action32,action33, action34);

			
			// action := PARALLEL(action8);	
			// RETURN dOutput;
