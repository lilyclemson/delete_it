...
EXPORT yinyang_optm_test1 := MODULE
    ...
    dClusterCounts := TABLE(dUpperBound, {y; UNSIGNED c := COUNT(GROUP);},y );
		...
		lInput fIterate(DATASET(lInput) d,UNSIGNED c):=FUNCTION	
		  action00 := OUTPUT(c, NAMED('iteration'));
			...
			deltaG1 := DEDUP(SORT(DISTRIBUTE(dLbsItr,x), x,value, LOCAL), x, LOCAL);
      ...
      action77 :=OUTPUT(dMap_gf, NAMED('YinyangKMeansv4_dMap_gf'));			
			dMappedDistances_gf := MappedDistances(d01,dCentroidIn,fDist,dMap_gf);
			action777 :=OUTPUT(dMappedDistances_gf, NAMED('YinyangKMeansv4_dMappedDistances_gf'));	

			
      gf_pass := JOIN(gf_pass_pre, dMappedDistances_gf, LEFT.x = RIGHT.x AND LEFT.value < RIGHT.value, TRANSFORM(LEFT));
			action88 := OUTPUT(gf_pass, NAMED('YinyangKMeansv4_gf_pass'));
			groupFilter := DEDUP(gf_pass,x);
      action99 :=OUTPUT(groupFilter, NAMED('YinyangKMeansv4_groupFilter'));	
/*
			dLbsGroupFilter := JOIN(dLbsItr, dDeltaG, LEFT.y = RIGHT.y , TRANSFORM(Mat.Types.Element, SELF.value := LEFT.value - RIGHT.value; SELF := LEFT), LOOKUP);
      action7 :=OUTPUT(dLbsGroupFilter, NAMED('YinyangKMeans_dLbsGroupFilter'));			
	
			groupFilter := DEDUP(SORT(DISTRIBUTE(JOIN(dUbGroupFilter, dLbsGroupFilter,LEFT.x = RIGHT.x AND RIGHT.value - LEFT.value < 0, TRANSFORM({UNSIGNED4 x}, SELF.x := LEFT.x)),x),x,LOCAL),x, LOCAL);
      ...
      lemma2 := DEDUP(SORT(DISTRIBUTE(JOIN(lemma2_r, lemma2_l_glb, LEFT.x = RIGHT.x AND LEFT.value <= RIGHT.value, TRANSFORM(LEFT), LOOKUP),x),x, LOCAL),x, LOCAL);	      
