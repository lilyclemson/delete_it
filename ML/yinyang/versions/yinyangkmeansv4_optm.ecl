...
EXPORT yinyangkmeansv4_optm := MODULE
...
//on each node, the data points are sorted by x, value and then dedup based on x
    ...
    dClusterCounts := TABLE(dUpperBound, {y; UNSIGNED c := COUNT(GROUP);},y );
      ...
      action18 :=OUTPUT(dMap, NAMED('YinyangKMeansv4_dMap'));			
			...
			dLbsLocalFilter := JOIN(dDistancesLocalFilter, LocalFilter, LEFT.x = RIGHT.x AND LEFT.y != RIGHT.y, TRANSFORM(LEFT), LOOKUP);
      action_dLbsLocalFilter := OUTPUT(dLbsLocalFilter, NAMED('dLbsLocalFilter'));
      dGroupLbsLocalFilter := JOIN(dLbsLocalFilter, Gt, LEFT.y = RIGHT.x, TRANSFORM(Mat.Types.Element,SELF.y := RIGHT.y, SELF := LEFT), MANY LOOKUP);		
      action_dGroupLbsLocalFilter := OUTPUT(dGroupLbsLocalFilter, NAMED('dGroupLbsLocalFilter'));
		  dGroupLbsUpdate := DEDUP(SORT(DISTRIBUTE(dGroupLbsLocalFilter,x),x, y, value, LOCAL),LEFT.x = RIGHT.x AND LEFT.y = RIGHT.y, LOCAL);      
      action_dGroupLbsUpdate := OUTPUT(dGroupLbsUpdate, NAMED('dGroupLbsUpdate'));
			...
			dLbsUpdate := JOIN(dLbsGroupFilter, dGroupLbsUpdate,LEFT.x = RIGHT.x AND LEFT.y = RIGHT.y, TRANSFORM(Mat.Types.Element, SELF.value := IF( RIGHT.x = LEFT.x,RIGHT.value,LEFT.value ), SELF.y := IF(RIGHT.x = LEFT.x, RIGHT.y,LEFT.y), SELF := LEFT), LEFT OUTER, LOCAL);
      ...
      action21 :=OUTPUT(SORT(dLbsUpdate,x), NAMED('YinyangKMeansv4_dLbsUpdate'));
          ...
          action34 :=OUTPUT(dCentroid2, NAMED('YinyangKMeansv4_dCentroid2'));	
    			...
    			// action := PARALLEL(action_dLbsLocalFilter,action_dGroupLbsLocalFilter,output_dDistances0, output_dUpperBound, output_Gt, output_groups, output_dLowerBound, action00,action0,action1,action2, action3,output_dGroupDeltaC,action5,action6,action66,action7,action8,action9,action10,action11,action12,action13,action_dGroupLbsUpdate,action14,action15,action16,action17,action18,action19,action20,action21,action22,action23,action24,action25,action26,action27,action28,action29,action30,action31,action32,action33, action34);
			// action := PARALLEL(action7,action200, action21);	
			RETURN dOutput;
			// RETURN WHEN(dOutput, action30);
		END;
