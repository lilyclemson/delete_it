...
//%%%%%%%%%  code added for v4_update
      ...
      gf_pass := JOIN(dUbGroupFilter, dLbsGroupFilter,LEFT.x = RIGHT.x AND RIGHT.value - LEFT.value < 0, TRANSFORM({UNSIGNED4 x}, SELF.x := LEFT.x));
	    action_gf_pass := OUTPUT(gf_pass, named('gf_pass'));
...
// Global SORT (expensive)& DEDUP : 
//run a few tests to see which works best
//1. distribute dataset by X (the attribute you wanna sort)
//2. DEDUP and SORT should be LOCAL

//Global JOIN & SORT are the key nobs: solving these nobs are the major way to resolve the time consuming operations.

//SMART(automatic) VS LOOKUP
			...
			lemm2_pre := JOIN(lemma2_r, lemma2_l_glb, LEFT.x = RIGHT.x AND LEFT.value <= RIGHT.value, TRANSFORM(LEFT), LOOKUP);	      
			action_lemma2_pre := OUTPUT(lemm2_pre, named('lemm2_pre'));
      ...
      action18 :=OUTPUT(dMap, NAMED('YinyangKMeansv4_dMap'));			
			...
			dLbsLocalFilter := JOIN(LocalFilter,dDistancesLocalFilter, LEFT.x = RIGHT.x AND LEFT.y != RIGHT.y , TRANSFORM(RIGHT));
      output_dLbsLocalFilter := OUTPUT(dLbsLocalFilter, NAMED('dLbsLocalFilter'));
		  ...
		  output_dGroupLbsLocalFilter := OUTPUT(dGroupLbsLocalFilter, NAMED('dGroupLbsLocalFilter'));

			dGroupLbsUpdate := DEDUP(dGroupLbsLocalFilter,LEFT.x = RIGHT.x AND LEFT.y = RIGHT.y);      
      output_dGroupLbsUpdate := OUTPUT(dGroupLbsUpdate, NAMED('dGroupLbsUpdate'));

			dLbsUpdate := JOIN(dLbsGroupFilter, dGroupLbsUpdate,LEFT.x = RIGHT.x AND LEFT.y = RIGHT.y, TRANSFORM(Mat.Types.Element, SELF.value := IF( LEFT.x= RIGHT.x, RIGHT.value,LEFT.value), SELF := LEFT;), LEFT OUTER, LOOKUP );
...
*/
    			...
    			action := PARALLEL(output_dLbsLocalFilter,output_dGroupLbsLocalFilter,output_dGroupLbsUpdate ,action_lemma2_pre,output_dDistances0, output_dUpperBound, output_Gt, output_groups, output_dLowerBound, action00,action0,action1,action2, action3,action4,output_dGroupDeltaC,action_gf_pass,action5,action6,action7,action8,action9,action10,action11,action12,action13,action14,action15,action16,action17,action18,action19,action20,action21,action22,action23,action24,action25,action26,action27,action28,action29,action30,action31,action32,action33, action34);
